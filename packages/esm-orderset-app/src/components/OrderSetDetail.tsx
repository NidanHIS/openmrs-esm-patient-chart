      ...
      <div className={styles.contentGrid}>
        ...
        <div className={styles.sideColumn}>
          <div className={styles.infoPanel}>
            ...
            {set.auditInfo?.creator && (
              <div className={styles.infoRow}>
                <span className={styles.infoLabel}>{t('createdBy', 'Created By')}:</span>
                <span className={styles.infoValue}>{set.auditInfo.creator.display}</span>
              </div>
            )}
            {set.auditInfo?.dateCreated && (
              <div className={styles.infoRow}>
                <span className={styles.infoLabel}>{t('dateCreated', 'Date Created')}:</span>
                <span className={styles.infoValue}>
                  {new Date(set.auditInfo.dateCreated).toLocaleDateString()}
                </span>
              </div>
            )}
            {set.auditInfo?.changedBy && (
              <div className={styles.infoRow}>
                <span className={styles.infoLabel}>{t('changedBy', 'Changed By')}:</span>
                <span className={styles.infoValue}>{set.auditInfo.changedBy.display}</span>
              </div>
            )}
            {set.auditInfo?.dateChanged && (
              <div className={styles.infoRow}>
                <span className={styles.infoLabel}>{t('dateChanged', 'Date Changed')}:</span>
                <span className={styles.infoValue}>
                  {new Date(set.auditInfo.dateChanged).toLocaleDateString()}
                </span>
              </div>
            )}
          </div>
