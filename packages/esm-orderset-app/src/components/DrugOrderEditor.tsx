...
export default function DrugOrderEditor({
  ...
  const DURATION_UNIT_DAYS_MAP: Record<string, number> = {
  ...
  const calculateQuantity = (item: OrderItem) => {
    if (item.asNeeded || !item.dose || !item.frequency || !item.duration || !item.durationUnit) {
      return item.quantity;
    }

    const freq = orderConfig.orderFrequencies.find((f) => f.valueCoded === item.frequency);
    const freqPerDay = (freq as any)?.frequencyPerDay;
    const durationMultiplier = DURATION_UNIT_DAYS_MAP[item.durationUnit];

    if (freqPerDay && durationMultiplier) {
      return Math.ceil(item.dose * freqPerDay * item.duration * durationMultiplier);
    }
  ...
  const handleUpdate = (id: string, updates: Partial<OrderItem>) => {
    const currentItem = drugs.find((d) => d.id === id);
    if (!currentItem) return;

    let enrichedUpdates = { ...updates };

    if (updates.doseUnit && orderConfig) {
      const doseUnitConfig = orderConfig.drugDosingUnits.find((u) => u.valueCoded === updates.doseUnit);
      enrichedUpdates.doseUnitConceptId = doseUnitConfig?.conceptId;

      // Sync quantity units if they were matching the old dose unit or if not set yet
      if (
        currentItem.memberType === 'DRUG' &&
        (!currentItem.quantityUnits || currentItem.quantityUnits === currentItem.doseUnit)
      ) {
        enrichedUpdates.quantityUnits = updates.doseUnit;
        enrichedUpdates.quantityUnitsConceptId = doseUnitConfig?.conceptId;
      }
    }
    ...
    if (updates.frequency && orderConfig) {
      enrichedUpdates.frequencyConceptId = orderConfig.orderFrequencies.find((f) => f.valueCoded === updates.frequency)
        ?.conceptId;
    }
    if (updates.durationUnit && orderConfig) {
      enrichedUpdates.durationUnitConceptId = orderConfig.durationUnits.find((u) => u.valueCoded === updates.durationUnit)
        ?.conceptId;
    }
    if (updates.quantityUnits && orderConfig) {
      // User requested to use doseunits as quantity units for drugs for now
      enrichedUpdates.quantityUnitsConceptId = orderConfig.drugDosingUnits.find(
        (u) => u.valueCoded === updates.quantityUnits,
      )?.conceptId;
    }

    const updatedItem = { ...currentItem, ...enrichedUpdates };

    // Auto-calculate quantity for drugs if relevant fields changed
    if (
      updatedItem.memberType === 'DRUG' &&
      (updates.dose !== undefined ||
        updates.frequency !== undefined ||
        updates.duration !== undefined ||
        updates.durationUnit !== undefined ||
        updates.asNeeded !== undefined)
    ) {
      updatedItem.quantity = calculateQuantity(updatedItem);
    }

    onChange(drugs.map((d) => (d.id === id ? updatedItem : d)));
  };
  ...
  const addItem = (
    name: string,
    type: OrderMemberType,
    uuid?: string,
    conceptId?: number,
    dosageForm?: { uuid: string; display: string },
  ) => {
    const matchedDoseUnit = dosageForm
      ? orderConfig.drugDosingUnits.find((u) => u.valueCoded === dosageForm.uuid)
      : undefined;
    const defaultDoseUnit = matchedDoseUnit || orderConfig.drugDosingUnits[0];
    const defaultRoute =
    ...
    const defaultDurationUnit =
      orderConfig.durationUnits.find((u) => u.value?.toLowerCase() === 'days') || orderConfig.durationUnits[0];
    ...
    const newItem: OrderItem = {
      ...
      quantityUnitsConceptId:
        type === 'DRUG' || type === 'MEDICAL_SUPPLY' ? defaultDoseUnit?.conceptId : undefined,
              ...
              <>
                {filteredAddItems.map((item) => (
                  <button
                    ...
                    onClick={() => addItem(item.name, item.type, item.uuid, item.id, (item as any).dosageForm)}
                  >
