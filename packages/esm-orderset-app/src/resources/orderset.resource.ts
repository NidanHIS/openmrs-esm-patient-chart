...
export function useOrderSets() {
  ...
  const mappedOrderSets: OrderSet[] = data?.data?.results.map((nos) => ({
    ...
    auditInfo: nos.auditInfo ? {
      creator: nos.auditInfo.creator ? {
        display: nos.auditInfo.creator.display,
        uuid: nos.auditInfo.creator.uuid,
      } : undefined,
      dateCreated: nos.auditInfo.dateCreated,
      changedBy: nos.auditInfo.changedBy ? {
        display: nos.auditInfo.changedBy.display,
        uuid: nos.auditInfo.changedBy.uuid,
      } : undefined,
      dateChanged: nos.auditInfo.dateChanged,
    } : undefined,
  })) ?? [];
