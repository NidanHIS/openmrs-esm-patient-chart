...
export function useOrderConfig(): {
  ...
  const {
    ...
    `${restBaseUrl}/orderentryconfig?v=custom:(uuid,display,frequencyPerDay,concept:(id,names:(display,uuid)))`,
    openmrsFetch,
  );
  ...
  const results = useMemo(
    () => ({
      orderConfigObject: {
        ...
        orderFrequencies:
          frequencyData?.data?.orderFrequencies?.map(({ uuid, display, concept, frequencyPerDay }) => ({
            valueCoded: uuid,
            value: display,
